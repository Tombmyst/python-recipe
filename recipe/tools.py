...
from recipe.util import is_empty, call_or_return_value


def do(func: Callable, *func_args, **func_kwargs):
    """
    Generates a function that will call the given function *func* with provided arguments *func_args* and *func_kwargs*.
    """
    def _do(_value: Any, _sequence_name: str, _logger: logging.Logger, _on_error: OnError, **kwargs):
...
def do_and_take_index(func: Callable, *func_args, _index: int, **func_kwargs):
    """
    Similar to *do*. The difference is in the result of the function *func*, which in this case must return an iterable.
    On successful execution, the result of the function will be the element at index *_index* of the iterable.
...
def while_(*functions: Callable, _block_name: str = None, _condition: bool | Callable[[Any], bool] = True):
    """
    Executes the block of functions *functions* as long as the condition evaluates to True.

    *functions* must be tool functions found in this module or any other function that respect the same signature.
    ...
    def _while_(
        ...
        while modified != _value and \
                not is_empty(_value) and \
                call_or_return_value(_condition, _value):
...
def if_(
    ...
    """
    Conditionally executes a given block of functions depending on the result of *condition*.

    .. note :: Does not perform a condition check on the inner value.
    """
    ...
    def _if_(
        ...
        if call_or_return_value(condition, _processed):
