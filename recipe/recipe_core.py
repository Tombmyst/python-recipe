...
from recipe.types_ import RecipeResult, NULL
...
class Recipe:
    ...
    def __init__(self, name: str, *functions: Callable, cachable: bool = True, on_error_behavior: OnError = OnError.LOG):
        self.name: str = name
    ...
    def __call__(
        self, initial_value: Any, logger: logging.Logger = None
    ) -> RecipeResult:
        ...
        result: RecipeResult = execute_sequence_of_functions(self.name, self.functions, initial_value, logger,
                                                             self.on_error_behavior, is_mutable(initial_value))
        self._cache[initial_value] = result
        logger.debug(
            f'Recipe "%s" -> Successfully applied recipy (from "%s" to "%s" (Last = "%s"))',
            self.name,
            initial_value,
            result.actual_value,
            result.last_value
...
def execute_sequence_of_functions(
        ...
        on_error_behavior: OnError,
        value_is_mutable: bool = False
) -> RecipeResult:
    processed: Any = copy(initial_value) if value_is_mutable else initial_value
    last: Any = None
    logger.debug('Recipe "%s" -> Initial Value%s: "%s"', sequence_name, ' (MUT) ' if value_is_mutable else '', initial_value)
    ...
    for func in tool_functions:
        ...
        try:
            processed = func(
                ...
                _logger=logger,
                _on_error=on_error_behavior
            )
            logger.debug('Recipe "%s" -> %s: "%s"',
                         sequence_name,
            ...
            handle_error(
                ...
                message=f'Recipe {sequence_name} -> '
                        f'An error occurred while applying "{get_tool_function_name(func)}" on value "{processed}"'
            )
