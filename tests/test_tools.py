...
from recipe.tools import do, do_and_take_index, if_, while_
...
def test_do():
    ...
    assert (
        second_with_kwarg("ROGERS-POTATO", "sequence", logging.getLogger(), OnError.LOG) == "ROGER"
    )
...
def test_while():
    ...
    while_empty = while_(do(_crop, how_much=2), do(_add, what="-"), _block_name="test")
    ...
    while_error = while_(
        do(_crop, how_much=2), do(_add, what="-"), do(_raiser), _block_name="test3"
    )
...
def test_if():
    ...
    if_true = if_(True, _on_true=[do(_on_true)], _on_false=[do(_on_false)], _block_name="test1")

    if_false = if_(False, _on_true=[do(_on_true)], _on_false=[do(_on_false)], _block_name="test2")

    if_raiser = if_(True, _on_true=[do(_raiser)], _block_name="test3")

    assert (
        if_true("rogers-PoTatO", "sequence", logging.getLogger(), OnError.LOG)
        == "rogers-PoTatO patate"
    )
    assert (
        if_false("rogers-PoTatO", "sequence", logging.getLogger(), OnError.LOG)
        == "rogers-PoTatO rogers"
    )

    assert (
        if_raiser("rogers-PoTatO", "sequence", logging.getLogger(), OnError.LOG) == "rogers-PoTatO"
    )
    with pytest.raises(Exception):
