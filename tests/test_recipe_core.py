...
recipe = Recipe('test',
                ...
                while_(
                    do(lambda x: x[0: -1]),
                    _condition=lambda x: len(x) > 2
                )
                )


recipe_cache = Recipe('test',
                      do(str.lower),
                      do_and_take_index(str.split, '-', _index=0),
                      do(lambda x: x + ' patate'),
                      while_(
...
def test_recipe_cache():
    ...
    assert result == result2  # but we know that value was changed in _get_value function, this proves the cache is working
...
def test_recipe_on_bad_initial_value():
    ...
    assert str(result.error) == str(TypeError("descriptor 'lower' for 'str' objects doesn't apply to a 'int' object"))
